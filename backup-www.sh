...
# OPTIONS
#----------------------------------------
DAYS_TO_KEEP=28    # 0 to keep forever
WWW_PATH='/home/klient.dhosting.pl/bumex012/bumex.pl'
BACKUP_PATH='/home/klient.dhosting.pl/bumex012/backup/bumex'
#----------------------------------------
...
for website in * ; do
  if [[ -d $website && ! -L "$website" ]]; then
    ...
    date=$(date -I)
    mkdir /home/klient.dhosting.pl/bumex012/backup/bumex/$date-$website/
    tar -cvpzf $BACKUP_PATH/$date-$website/$date-$website.tar.gz $website
  fi
done
